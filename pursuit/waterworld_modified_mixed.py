...
class MAWaterWorld_mod_mixed(AbstractMAEnv, EzPickle):
    ...
    def __init__(self, n_pursuers, n_evaders, n_coop=2, n_poison=10, radius=0.015,
        ...
        self.seed()
        self._pursuers = [
            Archea(npu + 1, self.radius * 1.7, self.n_sensors, self.sensor_range[npu], addid=self._addid,
        ...
        self._poisons = [
            Archea(npo + 1, self.radius * 2, self.n_poison, 0) for npo in range(self.n_poison)
        ]
    ...
    def reset(self):
        ...
        for pursuer in self._pursuers:
            ...
            pursuer.set_position(self._respawn(pursuer.position,
                                               pursuer._radius))
        ...
        for evader in self._evaders:
            ...
            s = (self.np_random.rand(2) - 0.5)
            s += 0.01
            s = s / np.linalg.norm(s)
            evader.set_velocity(s * self.ev_speed)  # TODO policies
        ...
        for poison in self._poisons:
            ...
            s = s + 0.01
            s = s / np.linalg.norm(s)
            poison.set_velocity(s * self.ev_speed)

        return self.step(np.zeros((self.n_pursuers, 1)))[0]
    ...
    def step(self, action_Np2):
        action_Np2 = np.asarray(action_Np2).astype(int)
        action_Np_2 = action_Np2.reshape((self.n_pursuers, 1))
        # Players
        time_of_catch = 0
        rewards = np.zeros((self.n_pursuers,))
        assert action_Np_2.shape == (self.n_pursuers, 1)

        actions = np.array([[-0.02, 0],
                            [0.02, 0], [0, -0.02], [0, 0.02]])

        for npu, pursuer in enumerate(self._pursuers):
            pursuer.set_position(pursuer.position +
                                 actions[action_Np_2[npu, 0]])

        # Players stop on hitting a wall
        for npu, pursuer in enumerate(self._pursuers):
            clippedx_2 = np.clip(pursuer.position, 0.05, 0.95)
        ...
        if self._speed_features:
            sensorfeatures_Np_K_O = np.c_[sensed_evdistfeatures_Np_K,
        ...
        return obslist, rewards, done, info, time_of_catch

    def render(self, screen_size=84, rate=10, mode='human'):
        import cv2
        ...
        # Pursuers
        for npu, pursuer in enumerate(self._pursuers):
            color = [(255, 255, 0), (255, 0, 0), (192, 192, 192),
                     (255, 165, 0)]
            cv2.circle(img,
                       tuple((pursuer.position * screen_size).astype(int)),
                       int(pursuer._radius * screen_size),
                       color[npu], -1, lineType=cv2.LINE_AA)
        # Evaders
        for evader in self._evaders:
            color = (0, 255, 0)
            cv2.circle(img,
                       tuple((evader.position * screen_size).astype(int)),
                       int(evader._radius * screen_size),
        ...
        for poison in self._poisons:
            ...
            cv2.circle(img,
                       ...
                       int(poison._radius * screen_size),
                       color, -1, lineType=cv2.LINE_AA)
